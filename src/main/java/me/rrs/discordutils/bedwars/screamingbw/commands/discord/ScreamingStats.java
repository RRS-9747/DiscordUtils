...
public class ScreamingStats extends ListenerAdapter {
    ...
    private final PlayerStatisticsManager manager = BedwarsAPI.getInstance().getStatisticsManager();
        ...
        if (config.getBoolean("Stats.Kills.Enable"))
            builder.addField(config.getString("Stats.Kills.Name"),
                    config.getString("Stats.Kills.Value")
                            .replace("{KILLS}", String.valueOf(statistic.getKills())), true);
        ...
        if (config.getBoolean("Stats.Death.Enable"))
            builder.addField(config.getString("Stats.Death.Name"),
                    config.getString("Stats.Death.Value")
                            .replace("{DEATH}", String.valueOf(statistic.getDeaths())), true);

        if (config.getBoolean("Stats.KD.Enable"))
            builder.addField(config.getString("Stats.KD.Name"),
                    config.getString("Stats.KD.Value")
                            .replace("{KD}", String.valueOf(statistic.getKD())), true);

        if (config.getBoolean("Stats.Win.Enable"))
            builder.addField(config.getString("Stats.Win.Name"),
                    config.getString("Stats.Win.Value")
                            .replace("{WIN}", String.valueOf(statistic.getWins())), true);
        ...
        if (config.getBoolean("Stats.Losses.Enable"))
            builder.addField(config.getString("Stats.Losses.Name"),
                    config.getString("Stats.Losses.Value")
                            .replace("{LOSSES}", String.valueOf(statistic.getLoses())), true);

        if (config.getBoolean("Stats.RoundsPlayed.Enable"))
            builder.addField(config.getString("Stats.RoundsPlayed.Name"),
                    config.getString("Stats.RoundsPlayed.Value")
                            .replace("{ROUNDSPLAYED}", String.valueOf(statistic.getGames())), true);
        ...
        if (config.getBoolean("Stats.BedDestroyed.Enable"))
            builder.addField(config.getString("Stats.BedDestroyed.Name"),
                    config.getString("Stats.BedDestroyed.Value")
                            .replace("{BEDDESTROYED}", String.valueOf(statistic.getDestroyedBeds())), true);

        if (config.getBoolean("Stats.Score.Enable"))
            builder.addField(config.getString("Stats.Score.Name"),
                    config.getString("Stats.Score.Value")
                            .replace("{SCORE}", String.valueOf(statistic.getScore())), true);
